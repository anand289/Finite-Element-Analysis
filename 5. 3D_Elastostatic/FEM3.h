...
void FEM<dim>::define_boundary_conds(){
  ...
  const unsigned int totalDOFs = dof_handler.n_dofs(); //Total number of degrees of freedom
    ...
    if(problem==1){
      ...
      if (x_1==1 && nodalDOF==1)
      {
        boundary_values[global_dof] = ((0.5) + ((x_2)-(0.5))*(cos(PI/30)) - ((x_3) - (0.5))*(sin(PI/30)) - (x_2));
      }
      else if(x_1==1 && nodalDOF==2)
      {
        boundary_values[global_dof] = ((0.5) + ((x_2)-(0.5))*(sin(PI/30)) + ((x_3) - (0.5))*(cos(PI/30)) - (x_3));
      }
...
double FEM<dim>::torque(){
	...
	for (;elem!=endc; ++elem){
		...
		for (unsigned int f=0; f < faces_per_elem; f++){
          ...
          double arm_x2 = fe_face_values.quadrature_point(q)[1];   //EDIT
          double arm_x3 = fe_face_values.quadrature_point(q)[2];   //EDIT
          ...
          for( unsigned int k =0;k<dim;k++){
            for(unsigned int l = 0;l<dim;l++){
                  sigma_12 += C(0,1,k,l)*0.5*(du_dx[k][l]+du_dx[l][k]);
                  sigma_13 += C(0,2,k,l)*0.5*(du_dx[k][l]+du_dx[l][k]);
            }
          }

            torque += ((arm_x2)*(sigma_13) - (arm_x3)*(sigma_12))*fe_face_values.JxW(q);
