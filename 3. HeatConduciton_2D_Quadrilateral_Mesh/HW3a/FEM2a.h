...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2){
  ...
  double N = 0.; //Store the value of the basis function in this variable
...
void FEM<dim>::generate_mesh(std::vector<unsigned int> numberOfElements){
  ...
  double x_max = 0.03; //EDIT
  double  y_min = 0.; //EDIT
  double  y_max = 0.08; //EDIT
...
void FEM<dim>::define_boundary_conds(){
    ...
    double f, k =385.0 , c = (1./3.) ,c_bar = 8.0;
  ...
  const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes
    ...
    for (unsigned int globalNode = 0; globalNode < totalNodes; globalNode++)
    {
      ...
      {
        boundary_values[globalNode] = 300.*(1.+(c*nodeLocation[globalNode][0]));
      }
      if (nodeLocation[globalNode][1] == y_max)
      {
          boundary_values[globalNode] = 310.*(1.+(c_bar*nodeLocation[globalNode][0]*nodeLocation[globalNode][0]));
      }
    }
    ...
    for (unsigned int globalNode = 0; globalNode < totalNodes; globalNode++)
    {
      if (nodeLocation[globalNode][1] == y_min)  //bottom_boundary
      {
        boundary_values[globalNode] = 100+((f/(4.*k))*(nodeLocation[globalNode][0]*nodeLocation[globalNode][0]));
      }
      if (nodeLocation[globalNode][1] == y_max) //top_boundary
      {
          boundary_values[globalNode] = 100+((f/(4.*k))*(nodeLocation[globalNode][0]*nodeLocation[globalNode][0]+0.0064));
      }
      if (nodeLocation[globalNode][0] == x_min) //left_boundary
      {
          boundary_values[globalNode] = 100+((f/(4.*k))*(nodeLocation[globalNode][1]*nodeLocation[globalNode][1]));
      }
      if (nodeLocation[globalNode][0] == x_max) //right_boundary
      {
          boundary_values[globalNode] = 100+((f/(4.*k))*(nodeLocation[globalNode][1]*nodeLocation[globalNode][1]+0.0009));
      }
    }
  	//EDIT - Define the Dirichlet boundary conditions.
	}
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
      ...
      for (unsigned int q2 = 0; q2 < quadRule; q2++) {
        ...
        for(unsigned int A = 0;A<dofs_per_elem;A++){
          if (prob==2){
            ...
            Flocal[A] += basis_function(A,quad_points[q1],quad_points[q2])*f*detJ*quad_weight[q1]*quad_weight[q2];
          }
          //Code For non-zero Flocal
        }
    ...
    for (unsigned int q1 = 0; q1 < quadRule; q1++) {
      for (unsigned int q2 = 0; q2 < quadRule; q2++) {
        //Find Jacobian at quadrature point
        Jacobian = 0.;
       for (unsigned int i = 0; i < dim; i++) {
          for (unsigned int j = 0; j < dim; j++) {
            for(unsigned int A=0; A<dofs_per_elem;A++){
        ...
        for (unsigned int B = 0; B < dofs_per_elem; B++) {
          for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
              for (unsigned int I = 0; I < dim; I++) {
                for (unsigned int J = 0; J < dim; J++) {
	          //Klocal[i][j] += invJacob[i][I]*kappa[I][J]*invJacob[j][J]*detJ*quad_weight[q1]*quad_weight[q2];	
		  //Klocal[A][B] += (-1.)*(basis_gradient(A,quad_points[q1],quad_points[q2])[i]*invJacob[i][I] + basis_gradient(A,quad_points[q1],quad_points[q2])[j]*invJacob[j][I])
		                 // *kappa[I][J]*
                        	  //     (basis_gradient(B,quad_points[q1],quad_points[q2])[i]*invJacob[i][J] + basis_gradient(B,quad_points[q1],quad_points[q2])[j]*invJacob[j][J])
		                 // *detJ*
  ...
  F[local_dof_indices[A]] += Flocal[A];      
      //We will assemble F here if it were nonzero
      for (unsigned int B = 0; B < dofs_per_elem; B++) {
      ...
      double  f=-10000.;
       for (unsigned int i = 0; i < dim; i++) {
        ...
        detJ = Jacobian.determinant();
          ...
          u_exact = 100 + (f/(4.*k))*(x*x+y*y);
          l2norm += (u_h - u_exact)*(u_h - u_exact)*detJ*quad_weight[q1]*quad_weight[q2];
