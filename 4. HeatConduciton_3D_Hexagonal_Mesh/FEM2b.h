...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  std::vector<double> values(dim,0.0); //Store the value of the gradient of the basis function in this variable
    ...
    if (node==0) {
    values[0] = (-1./4.)*(1.-xi_2)*(1-xi_3);
    values[1] = (-1./4.)*(1.-xi_1)*(1-xi_3);
    values[2] = (-1./4.)*(1.-xi_1)*(1-xi_2);
  }
  else if (node == 1) {
    values[0] = (1./4.)*(1.-xi_2)*(1-xi_3);
    values[1] = (-1./4.)*(1.+xi_1)*(1-xi_3);
...
void FEM<dim>::define_boundary_conds(){
  ...
  f=0.;
    for (unsigned int globalNode = 0; globalNode < totalNodes; globalNode++)
    {
      ...
      {
        boundary_values[globalNode] = 300.*(1.+(c*(nodeLocation[globalNode][1]+nodeLocation[globalNode][2])));
      }
      if (nodeLocation[globalNode][0] == x_max)
      {
          boundary_values[globalNode] = 310.*(1.+(c*(nodeLocation[globalNode][1]+nodeLocation[globalNode][2])));
      }
    }
