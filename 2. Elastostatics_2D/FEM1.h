...
double FEM<dim>::basis_function(unsigned int node, double xi)
{
  ...
  for(int b=0 ; b<= basisFunctionOrder ; b++){
    if(b==node)                                                                                                           //definin basic function
      {continue;}
      N *= (xi - xi_at_node(b))/(xi_at_node(node)-xi_at_node(b));
     }

   //Store the value of the basis function in this variable
...
double  N_grad=0.;
  ...
  for(int c=0;c<=basisFunctionOrder;c++){
     if(c==node)
      {continue;}
       double N_num=1.;                                   
    for(int b=0;b<=basisFunctionOrder;b++){
      if(b==node||c==b)
	{continue;}                                                                                                               //defining basic gradient
      N_num *= (xi - xi_at_node(b))/(xi_at_node(node)-xi_at_node(b));
      }
      N_grad += N_num/(xi_at_node(node)- xi_at_node(c));
     
  }


  double value = N_grad; //Store the value of the gradient of the basis function in this variable
...
void FEM<dim>::assemble_system()
{
  ...
  for (; elem != endc; ++elem)
  {
    ...
    double E = 1e11, Ar = 0.0001, f_dash = 1e11, f_cap = 1e12, h = 1e6;                                                                //constant defination 
    ...
    for (unsigned int A = 0; A < dofs_per_elem; A++)
    {
      for (unsigned int q = 0; q < quadRule; q++)
      {
		...
		if(prob == 1 || prob == 2){
	  Flocal[A] += ((Ar*h_e*f_dash)/2.)*basis_function(A,quad_points[q])*quad_weight[q]*x;                                                                             //flocal for problem 1 and 2
	}
	if(prob == 3|| prob == 4)
	  Flocal[A] += ((Ar*h_e*f_cap)/2.)*basis_function(A,quad_points[q])*quad_weight[q]*x*(L-x);                                                                         //flocal for problem 3 and 4
	//EDIT - Define Flocal.
    ...
    if (prob == 2 || prob == 4)
    {
      ...
      {
	Flocal[1] += h;                                                                                                                                                           //Neuman Boundary cindition
    ...
    for (unsigned int A = 0; A < dofs_per_elem; A++)
    {
      for (unsigned int B = 0; B < dofs_per_elem; B++)
      {
        for (unsigned int q = 0; q < quadRule; q++)
        {
		 Klocal[A][B] += (2.*E*Ar/h_e)*basis_gradient(A,quad_points[q])*basis_gradient(B,quad_points[q])*quad_weight[q];                                                                   // define Klocal
    ...
    for (unsigned int A = 0; A < dofs_per_elem; A++)
    {
      //EDIT - add component A of Flocal to the correct location in F
        F[local_dof_indices[A]]+= Flocal[A];                                                                                                                                //flocal to fglobal
	...
	corresponding to element node number A*/
	  ...
	  K.add(i,j,C);*/
		K.add(local_dof_indices[A],local_dof_indices[B],Klocal[A][B]);                                                                                                     //Klocal to Kglobal
...
double FEM<dim>::l2norm_of_error()
{
  ...
  for (; elem != endc; ++elem)
  {
	...
	u_exact= ((-f_dash*x*x*x)/(6.*E))+((f_dash*x*L*L)/(6.*E))+((x/L)*(g2-g1))+g1;
      }
      if (prob==2){
	  // u_exact = (-x*x*x/6)+(0.21/2)*x;
	  u_exact= (-f_dash*x*x*x)/(6.*E)+(h*x)/(E*Ar)+(f_dash*Ar*L*x*L)/(2.*E*Ar) +g1;
      }
      if (prob==3){
	// u_exact = (-x*x*x/6.)+(10.*x*x*x*x)/12.+(13./1200.)*x;
	u_exact= (-f_cap*x*x*x*L)/(6.*E)+(f_cap*x*x*x*x)/(12.*E) +(f_cap*x*L*L*L)/(12.*E)+(x/L)*(g2-g1)+g1;
      }
      if (prob==4){
	//u_exact = (-x*x*x/6)+(x*x*x*x)/(120.)+(61./600.)*x;
	u_exact= (-f_cap*x*x*x*L)/(6.*E)+(h*x)/(E*Ar)+(f_cap*x*x*x*x)/(12.*E)+(f_cap*x*L*L*L)/(6.*E) +g1;
      }
      l2norm += (u_exact - u_h)*(u_exact - u_h)*(h_e/2.)*quad_weight[q];
