...
void FEM<dim>::define_boundary_conds(){
  ...
  const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes
      ...
      double x_1 = nodeLocation[globalNode][0];
      double x_2 = nodeLocation[globalNode][1];
      double x_3 = nodeLocation[globalNode][2];
...
void FEM<dim>::solve_trans(){
  ...
  for(unsigned int t_step=1; t_step<3001; t_step++){
    ...
    D_tilde = D_trans;   
    double coeff_1 = (delta_t)*(1.0-alpha);                                    
    D_tilde.add(coeff_1,V_trans);
    ...
    D_trans = D_tilde;
    double coeff_2 = (delta_t)*(alpha); 
    D_trans.add(coeff_2,V_trans);
